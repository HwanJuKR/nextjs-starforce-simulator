describe('시뮬레이터', () => {
  ...
  describe('대량 강화', () => {
    it('10,000번 시뮬레이션 확인', () => {
      ...
      cy.get('[data-testid="attempt"]').invoke('text').then((text) => {
    ...
    it('완료 후 통계 업데이트 확인', () => {
      ...
      cy.get('[data-testid="cost"]').invoke('text').then((text) => {
  ...
  describe('이벤트 및 혜택 적용', () => {
    it('이벤트 체크박스 토글 → 확률/비용 변경 확인', () => {
      // 초기 비용 확인
      cy.get('[data-testid="enhance-cost"]').invoke('text').then((initialCostText) => {
        const initialCost = parseInt(initialCostText.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');

        // "비용 30% 할인" 이벤트 활성화
        cy.get('[data-testid="cost-discount"]').check();

        // 비용이 감소했는지 확인
        cy.get('[data-testid="enhance-cost"]').invoke('text').then((discountedCostText) => {
          const discountedCost = parseInt(discountedCostText.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');
        ...
        // 비용이 원래대로 돌아왔는지 확인
        cy.get('[data-testid="enhance-cost"]').invoke('text').then((restoredCostText) => {
          const restoredCost = parseInt(restoredCostText.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');
    ...
    it('MVP 등급 선택 → 비용 할인 확인', () => {
      ...
      // 초기 비용 확인
      let initialCost = 0;
      cy.get('[data-testid="enhance-cost"]').invoke('text').then((text) => {
        initialCost = parseInt(text.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');
      });
      ...
      // 비용이 감소했는지 확인
      cy.get('[data-testid="enhance-cost"]').invoke('text').then((text) => {
        const discountedCost = parseInt(text.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');
        expect(discountedCost).to.be.lessThan(initialCost);
      });
      ...
      cy.get('[data-testid="enhance-cost"]').invoke('text').then((text) => {
        const goldCost = parseInt(text.match(/[\d,]+/)?.[0]?.replace(/,/g, '') || '0');
